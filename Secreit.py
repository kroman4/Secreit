...
class TrainGenerator(object):
    ...
    def flow_from_directory(self, batch_size=36):
        '''
        Dict for # LabelEncode (convert class to int type)
        Extract the image path from the directory
        ...
        while(1>0):
            ...
            # Which file of D, E, P should it be? k is the file number
            ...
            # Repeat up to this point and store as many batch_sizes in the array (self.iamges, self.labels)
            # batch_sizeの数だけ格納されたら、戻り値として返し、配列(self.iamges, self.labels)を空にする
            # When the number of batch_size is stored, return it as a return value and empty the array (self.iamges, self.labels)
